...
export function Terminal({
  ...
  // Reset function to restart the animation loop
  const reset = () => {
  ...
  // Expose reset trigger on command change
  useEffect(() => {
    reset();
  }, [commands, outputs, initialLines]);
  ...
  return (
    ...
    >
      <div className="overflow-hidden rounded-lg border border-neutral-800 bg-neutral-900 shadow-2xl">
        ...
        <div
          ref={contentRef}
          className="no-visible-scrollbar h-[30rem] overflow-y-auto p-4 font-mono text-left"
        >
          {lines.map((line, i) => (
