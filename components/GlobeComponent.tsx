...
const companies = [
  { name: "Google", lat: 37.4220, lng: -122.0841, color: "#ea4335" },
  { name: "Stripe", lat: 37.7749, lng: -122.4194, color: "#635bff" },
  { name: "Meta", lat: 37.4851, lng: -122.2364, color: "#0668e1" },
  { name: "Microsoft", lat: 47.6739, lng: -122.1215, color: "#f25022" },
  { name: "Amazon", lat: 47.6062, lng: -122.3321, color: "#ff9900" },
  { name: "TCS", lat: 19.0760, lng: 72.8777, color: "#0083c9" },
  { name: "Infosys", lat: 12.9716, lng: 77.5946, color: "#007cc3" },
  { name: "Atlassian", lat: -33.8688, lng: 151.2093, color: "#0052cc" },
  { name: "Spotify", lat: 59.3293, lng: 18.0686, color: "#1ed760" },
  { name: "ASML", lat: 51.4039, lng: 5.4124, color: "#00a2e8" }
];
...
export function World(props: WorldProps) {
  ...
  return (
    ...
    >
      ...
      {/* 3D company markers with logo initials and labels */}
      {companies.map((company) => {
        const pos = convertLatLngToVector3(company.lat, company.lng, 102);
        return (
          <group key={company.name} position={pos}>
            <mesh>
              <sphereGeometry args={[1.2, 16, 16]} />
              <meshBasicMaterial color={company.color} toneMapped={false} />
            </mesh>
            <mesh rotation={[Math.PI / 2, 0, 0]}>
              <ringGeometry args={[1.6, 2.4, 32]} />
              <meshBasicMaterial color={company.color} side={2} opacity={0.6} transparent />
            </mesh>
            <Html distanceFactor={220} occlude={false} transform={false}>
              <div 
                className="flex items-center gap-1 bg-stone-950/95 border rounded px-1.5 py-0.5 select-none pointer-events-none whitespace-nowrap shadow-[0_0_10px_rgba(0,0,0,0.8)]"
                style={{ borderColor: `${company.color}60` }}
              >
                <span 
                  className="w-2.5 h-2.5 rounded-full flex items-center justify-center text-[7px] font-black text-white" 
...
export default function GlobeComponent() {
  ...
  // Arc database statically colors to avoid hydration issues in SSR
  const sampleArcs = [
  ...
  return (
    <div className="relative flex flex-col md:flex-row items-center justify-between rounded-3xl p-4 sm:p-6 md:p-10 border border-orange-500/20 bg-[#181615]/50 backdrop-blur-sm w-full mx-auto overflow-hidden shadow-[0_0_30px_rgba(249,115,22,0.03)] gap-6 md:gap-10">
      {/* Component Heading inside the boundary at the left */}
      <div className="text-left space-y-4 md:space-y-6 md:w-1/2 flex-shrink-0">
        <div className="space-y-2 lg:space-y-4">
          <span className="px-2.5 py-0.5 bg-orange-500/10 text-orange-400 border border-orange-500/20 rounded text-[9px] font-mono font-bold uppercase tracking-wider">
            Global Offer Readiness
          </span>
          <h2 className="font-headline-lg text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-mono text-white tracking-tight uppercase">Own Your Opportunity</h2>
          <p className="font-body-md text-stone-400 text-xs sm:text-sm md:text-base max-w-md leading-relaxed">
            Vector builds your preparation index to open up options at top organizations globally. Focus on real mock practice and cadence tracking to lock in the offer in your hands.
          </p>
        </div>
        ...
        <div className="space-y-4 pt-4 border-t border-orange-500/10 max-w-md">
          <div className="flex items-start gap-3">
            <span className="material-symbols-outlined text-orange-400 text-lg mt-0.5 select-none">location_on</span>
            <div className="space-y-0.5">
              <h4 className="text-xs sm:text-sm font-mono font-bold text-white uppercase">Geo-Aligned Target Prep</h4>
              <p className="text-stone-400 text-[11px] sm:text-xs font-body-md leading-relaxed">
                Mock rounds are configured based on regional target pathways, adapting to localized behavioral expectations and compensation standards.
              </p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <span className="material-symbols-outlined text-orange-400 text-lg mt-0.5 select-none">shield</span>
            <div className="space-y-0.5">
              <h4 className="text-xs sm:text-sm font-mono font-bold text-white uppercase">Vetted Placement Tracks</h4>
              <p className="text-stone-400 text-[11px] sm:text-xs font-body-md leading-relaxed">
                Access specialized paths derived from successful mock interviews and strategies that landed offers at top corporate headquarters.
              </p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <span className="material-symbols-outlined text-orange-400 text-lg mt-0.5 select-none">query_stats</span>
            <div className="space-y-0.5">
              <h4 className="text-xs sm:text-sm font-mono font-bold text-white uppercase">Offer Probability Index</h4>
              <p className="text-stone-400 text-[11px] sm:text-xs font-body-md leading-relaxed">
                Analyze speech pace, filler-word counters, layout parsing flags, and core skill requirements to forecast your placement readiness.
              </p>
            </div>
          </div>
        </div>
      </div>
      {/* 3D Canvas / Globe container on the right (square box to keep globe round) */}
      <div className="w-full md:w-1/2 aspect-square max-h-[20rem] sm:max-h-[24rem] md:max-h-[28rem] lg:max-h-[32rem] relative overflow-hidden select-none">
